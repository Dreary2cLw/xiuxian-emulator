...
export class TreasureCabinet extends plugin {
	...
	async qh(e) {
		...
		for (let i = 0; i < ass.藏宝阁.length; i++) {
			...
			if (isNotNull(thing_pinji)) {
				if (
					ass.藏宝阁[i].name == thingqq &&
					pinji2[ass.藏宝阁[i].pinji] == thing_pinji
				) {
					x = i;
					console.log(x);
					break;
	...
	async fr(e) {
		...
		if (thing_exist.class == '装备') {
			...
			let thing_quantity2 = najie.装备.find(
				(item) => item.name == thing_name && item.pinji == thing_pinji_number2
			).数量;
			if (thing_quantity2 < thing_acunot) {
				//不够
				e.reply(
					`你目前只有[${thing_name}【${
						pinji[
		...
		for (let i = 0; i < ass.藏宝阁.length; i++) {
			...
			if (ass.藏宝阁[i].name == thing_name) {
				if (thing_exist.class == '装备') {
					if (pinji[ass.藏宝阁[i].pinji] == thing_pinji) {
						//判断评级
						x = i;
						break;
					}
	...
	async duihuan(e) {
		...
		if (gonxianzhi > thing_whole) {
			...
			e.reply([
				`兑换成功!  获得[${thing_name}${msg}]x${thing_acunot},消耗了[${thing_whole}]贡献值,剩余[${
					gonxianzhi - thing_whole
				}]贡献值  `,
				'\n可以在【我的纳戒】中查看',
			]);
		} else {
	...
	async Feed_Beast(e) {
		...
		if (now_Time < last_time + feedTimeout) {
			...
			let Couple_s = Math.trunc(
				((last_time + feedTimeout - now_Time) % 60000) / 1000
			);
		...
		e.reply(
			`喂养成功，你和神兽的亲密度增加了${Math.trunc(
				(thing_exist.出售价 / 10000) * thing_value
			)},当前为${player.favorability}`
		);
		return;
	}
}
