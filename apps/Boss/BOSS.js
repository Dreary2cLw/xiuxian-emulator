...
export class BOSS extends plugin {
	...
	async WorldBossBattle(e) {
		...
		if (await data.existData('player', e.user_id)) {
			...
			await sleep(1000);
			if (
				TotalDamage >= 0.05 * WorldBossStatus.OriginHealth &&
				!WorldBossStatus.isWeak &&
...
async function InitWorldBoss(e) {
	...
	let Health = Math.trunc(X * 280 * player_quantity * 5);
    let Attack = Math.trunc(X * 220* player_quantity/10);
    let Defence = Math.trunc(X * 200* player_quantity/10);
	let Reward = Math.trunc(
