...
export class BOSS2 extends plugin {
			...
			if (WorldBossStatusStr != undefined) {
				...
				if (WorldBossStatus != undefined) {
					...
					let BOSSCurrentAttack = WorldBossStatus.isAngry
						? Math.trunc(WorldBossStatus.攻击 * 1.2)
						: WorldBossStatus.isWeak
						? Math.trunc(WorldBossStatus.攻击)
						: WorldBossStatus.攻击;
					let BOSSCurrentDefence = WorldBossStatus.isWeak
						? Math.trunc(WorldBossStatus.防御 * 0.8)
						: WorldBossStatus.防御;
					let ReplyMsg = [
					...
					if (WorldBossStatus.isWeak)
						ReplyMsg.push(`虚弱(还剩${WorldBossStatus.isWeak}回合)`);
					else if (WorldBossStatus.isAngry)
						ReplyMsg.push(`狂暴(还剩${WorldBossStatus.isAngry}回合)`);
	...
	async ShowDamageList(e) {
		if (await BossIsAlive()) {
			...
			let img = await get_tianlibang_img(e,msg);
			e.reply(img);
			await sleep(1000);
			if (CurrentQQ != undefined)
				e.reply(
					`你在天理周本贡献排行榜中排名第${CurrentQQ}，造成伤害${
	...
	async WorldBossBattle(e) {
		...
		if (await data.existData('player', e.user_id)) {
			...
			if (
				...
				CurrentPlayerAttributes.lunhui > 0
			) {
			...
			if (WorldBOSSBattleCD[e.user_id] != undefined) {
				let Seconds = Math.trunc(
					(300000 - (new Date().getTime() - WorldBOSSBattleCD[e.user_id])) / 1000
				);
			...
			while (CurrentPlayerAttributes.当前血量 > 0 && WorldBossStatus.当前血量 > 0) {
				...
				if (!(BattleFrame & 1)) {
					...
					if (yuansu.chaodao && Achaodaohuihe > 0) {
						Achaodaohuihe -= 1;
						msg.push(
							WorldBossStatus.名号 + '的抗性大大下降,虚弱状态剩余' + Achaodaohuihe + '回合'
						);
					...
					let Player_To_BOSS_Damage =
						Harm(CurrentPlayerAttributes.攻击 * 0.85, BOSSCurrentDefence) +
						Math.trunc(
							CurrentPlayerAttributes.攻击 * CurrentPlayerAttributes.灵根.法球倍率 +
								CurrentPlayerAttributes.防御 * 0.1
						);
					...
					if (
						...
						msg.push(
							'你知道你的实力弱小，所以你使用了秘技【神行雷】，但是你的境界还是太低了，只发挥出来5%'
						);
						...
						CurrentPlayerAttributes.学习的功法.indexOf('八品·鬼帝功') > -1 &&
						BattleFrame == 0
					) {
						...
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('八品·八荒剑法') > -1 &&
						BattleFrame == 2
					) {
						...
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('伪九品·第一魔功') > -1 &&
						BattleFrame == 2
					) {
						...
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('伪八品·二重梦之㱬') > -1 &&
						BattleFrame == 4
					) {
						...
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('八品·心禅不灭诀') > -1 &&
						BattleFrame == 4
					) {
						msg.push('你使用了心禅不灭诀【万剑归宗】');
						Player_To_BOSS_Damage = Math.trunc(Player_To_BOSS_Damage * 1.25);
					} else if (
						CurrentPlayerAttributes.灵根.name === '轮回道体' &&
						BattleFrame == 0
					) {
						msg.push(
							`${CurrentPlayerAttributes.名号} 使用了先天神通，轮回之力需要时间准备`
						);
						...
						CurrentPlayerAttributes.灵根.name === '灭道杀神体' &&
						BattleFrame == 12
					) {
						...
						msg.push(
							'你等了许久，终于【天理】疲劳，露出了破绽，你飞杀而去，但是【天理】使用了【混元】！！你的伤害被吸收了！'
						);
						...
						msg.push(
							'你如老猎人般屏息观察，终于看准【天理】身法中的一处缺陷，瞄准后用力一刺，正中其要害之处。'
						);
						Player_To_BOSS_Damage = Math.trunc(Player_To_BOSS_Damage * 2);
					} else if (Random >= 0.82) {
						msg.push(
							'你瞄了许久，看准时机放出一道凌厉剑气，结果【天理】使用了【幻剑】，你一头雾水'
						);
					...
					Player_To_BOSS_Damage = Math.trunc(
						Player_To_BOSS_Damage * SuperAttack + Math.random() * 30
					);
					...
					if (
						(WorldBossStatus.灵根.name == '仙之心·水' &&
							CurrentPlayerAttributes.灵根.name == '仙之心·木') ||
						(WorldBossStatus.灵根.name == '仙之心·木' &&
							CurrentPlayerAttributes.灵根.name == '仙之心·水')
					) {
						TotalDamage = CurrentPlayerAttributes.攻击 * 0.3 + TotalDamage;
					}
					TotalDamage = Player_To_BOSS_Damage + TotalDamage + 持续伤害;
					if (WorldBossStatus.当前血量 < 0) {
						WorldBossStatus.当前血量 = 0;
					}
					msg.push(
						`${CurrentPlayerAttributes.名号}${ifbaoji(
							SuperAttack
						)}造成伤害${Player_To_BOSS_Damage}，天理剩余血量${WorldBossStatus.当前血量}`
					);
					...
					if (Random < 0.015) {
						msg.push(
							'【天理】使用了超上古功法【唱，跳，rap】你被不知名的球体差点打的形神具灭'
						);
						BOSS_To_Player_Damage = Math.trunc(BOSS_To_Player_Damage * 2.2);
					} else if (
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('八品·避空') > -1 &&
						BattleFrame == 4
					) {
						...
						msg.push(
							`${CurrentPlayerAttributes.名号} 使用了转生神通【轮墓】！你的伤害无法生效！`
						);
						BOSS_To_Player_Damage = 0;
					} else if (
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('八品·桃花神功') > -1 &&
						BattleFrame == 5 &&
						Random > 0.66
					) {
						msg.push(
							`${CurrentPlayerAttributes.名号} 使用了【三生桃花！】让攻击慢慢变成了漫天桃花飞舞。`
						);
						BOSS_To_Player_Damage *= -0.2;
					} else if (
						CurrentPlayerAttributes.学习的功法 &&
						CurrentPlayerAttributes.学习的功法.indexOf('伪九品·魔帝功') > -1 &&
						BattleFrame == 3 &&
						Random > 0.5
					) {
						msg.push(
							`${CurrentPlayerAttributes.名号} 用了魔帝功【吞噬】吸收了伤害变成自己的血量`
						);
						...
						msg.push(
							'【天理】使用了【葬天剑】，这招你感受到了恐怖的能量，不过还好速度不快，但也稍受波及。'
						);
						BOSS_To_Player_Damage = Math.trunc(BOSS_To_Player_Damage * 0.3);
					} else if (Random < 0.3) {
					...
					if (yuansu.ranshao && Bgandianhuihe > 0) {
						...
						msg.push(
							CurrentPlayerAttributes.名号 + '烧了起来,受到了' + 持续伤害 + '的燃烧伤害'
						);
					}
					if (yuansu.gandian && Bgandianhuihe > 0) {
						...
						CurrentPlayerAttributes.当前血量 -= 持续伤害;
						msg.push(
							CurrentPlayerAttributes.名号 + '触电了,受到了' + 持续伤害 + '的感电伤害'
						);
					}
					...
					if (!WorldBossStatus.isAngry && BOSSCurrentAttack > WorldBossStatus.攻击)
						BOSSCurrentAttack = WorldBossStatus.攻击;
					if (!WorldBossStatus.isWeak && BOSSCurrentDefence < WorldBossStatus.防御)
						BOSSCurrentDefence = WorldBossStatus.防御;
					if (CurrentPlayerAttributes.当前血量 < 0) {
						CurrentPlayerAttributes.当前血量 = 0;
					}
					CurrentPlayerAttributes.防御 = afangyu;
					msg.push(
						`天理攻击了${CurrentPlayerAttributes.名号}，造成伤害${BOSS_To_Player_Damage}，${CurrentPlayerAttributes.名号}剩余血量${CurrentPlayerAttributes.当前血量}`
					);
				}
			...
			await sleep(1000);
			e.reply([
				`${CurrentPlayerAttributes.名号}攻击了天理，造成伤害${TotalDamage}，天理剩余血量${WorldBossStatus.当前血量}`,
			]);
			await sleep(1000);
			if (
				TotalDamage >= 0.1 * WorldBossStatus.血量上限 &&
				!WorldBossStatus.isWeak &&
				!WorldBossStatus.isAngry
			) {
				WorldBossStatus.isAngry = 20;
				e.reply(
					'这场战斗重创了天理，但也令其躁动不安而进入狂暴模式！\n天理攻击获得强化，持续20回合'
				);
			}
			if (
				!WorldBossStatus.isAngry &&
				!WorldBossStatus.isWeak &&
				Math.random() < BattleFrame * 0.015
			) {
			...
			if (CurrentPlayerAttributes.当前血量 == 0) {
				...
				if (Math.random() < BattleFrame * 0.012) {
					...
					if (HPFormBOSS > CurrentPlayerAttributes.血量上限)
						HPFormBOSS =
							CurrentPlayerAttributes.血量上限 - CurrentPlayerAttributes.当前血量;
			...
			if (WorldBossStatus.当前血量 == 0) {
				...
				e.reply([
					...
					'\n恭喜你亲手结果了天理的性命,为民除害，额外获得50000灵石奖励！并在天理身上翻到了' +
						weizhi[a].name +
						'!',
				]);
				...
				for (let i = 0; i < PlayerList.length; i++) {
					...
					if (i < Show_MAX) {
						...
						//Bot.logger.mark(
						//	`[天理周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励${Reward}`
						//);
						continue;
					} else {
						CurrentPlayer.灵石 += 30000;
						//Bot.logger.mark(
						//	`[天理周本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励30000`
						//);
						await data.setData('player', PlayerRecordJSON.QQ[PlayerList[i]], CurrentPlayer);
					}
					if (i == PlayerList.length - 1)
						Rewardmsg.push('其余参与的修仙者均获得30000灵石奖励！');
				}
...
async function InitWorldBoss() {
	...
	let Health = Math.trunc(X * 280 * player_quantity * 3);
    let Attack = Math.trunc(X * 220* player_quantity/50);
    let Defence = Math.trunc(X * 200* player_quantity/50);
...
export async  function get_tianlibang_img(e, msg) {
