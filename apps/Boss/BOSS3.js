  ...
  async DeleteWorldBoss3(e) {
	...
	async ShowDamageList(e) {
		if (await BossIsAlive()) {
			...
			if (CurrentQQ != undefined)
				e.reply(
					`你在初夏副本贡献排行榜中排名第${CurrentQQ}，造成伤害${
	...
	async WorldBossBattle(e) {
		...
		if (await data.existData('player', e.user_id)) {
			...
			while (CurrentPlayerAttributes.当前血量 > 0 && WorldBossStatus.当前血量 > 0) {
				...
				if (!(BattleFrame & 1)) {
					...
					if (
						...
						msg.push(
							'你等了许久，终于【初夏】疲劳，露出了破绽，你飞杀而去，但是【初夏】使用了【混元】！！你的伤害被吸收了！'
						);
						...
						msg.push(
							'你如老猎人般屏息观察，终于看准【初夏】身法中的一处缺陷，瞄准后用力一刺，正中其要害之处。'
						);
						Player_To_BOSS_Damage = Math.trunc(Player_To_BOSS_Damage * 2);
					} else if (Random >= 0.82) {
						msg.push(
							'你瞄了许久，看准时机放出一道凌厉剑气，结果【初夏】使用了【幻剑】，你一头雾水'
						);
					...
					msg.push(
						`${CurrentPlayerAttributes.名号}${ifbaoji(
							SuperAttack
						)}造成伤害${Player_To_BOSS_Damage}，初夏剩余血量${WorldBossStatus.当前血量}`
					);
					...
					let BOSS_To_Player_Damage = Harm(
						BOSSCurrentAttack,
						Math.trunc(CurrentPlayerAttributes.防御 * 0.2)
					);
					if (Random < 0.015) {
						msg.push(
							'【初夏】使用了超上古功法【唱，跳，rap】你被不知名的球体差点打的形神具灭'
						);
						...
						msg.push(
							'【初夏】使用了【葬天剑】，这招你感受到了恐怖的能量，不过还好速度不快，但也稍受波及。'
						);
						BOSS_To_Player_Damage = Math.trunc(BOSS_To_Player_Damage * 0.3);
					} else if (Random < 0.3) {
					...
					msg.push(
						`初夏攻击了${CurrentPlayerAttributes.名号}，造成伤害${BOSS_To_Player_Damage}，${CurrentPlayerAttributes.名号}剩余血量${CurrentPlayerAttributes.当前血量}`
					);
				}
			...
			await sleep(1000);
			e.reply([
				`${CurrentPlayerAttributes.名号}攻击了初夏，造成伤害${TotalDamage}，初夏剩余血量${WorldBossStatus.当前血量}`,
			]);
			await sleep(1000);
			if (
				...
				for (let i = 0; i < PlayerList.length; i++) {
					...
					if (i < Show_MAX) {
						...
						Bot.logger.mark(
							`[初夏副本] 结算:${
								PlayerRecordJSON.QQ[PlayerList[i]]
							}增加灵石奖励${Reward}并且获得仙子的赏识,获得道具【仙子邀约】x1`
						);

						await redis.set(
							'xiuxian:player:' + CurrentPlayer.id + ':Guanghangon',
							JSON.stringify(action2 + 1)
						);
						...
						Bot.logger.mark(
							`[初夏副本] 结算:${PlayerRecordJSON.QQ[PlayerList[i]]}增加奖励300000`
						);
						await data.setData('player', PlayerRecordJSON.QQ[PlayerList[i]], CurrentPlayer);
					}
					if (i == PlayerList.length - 1)
						Rewardmsg.push('其余参与的修仙者均获得300000灵石奖励！');
				}
...
async function BossIsAlive() {
	return (
		(await redis.get('Xiuxian:WorldBossStatus3')) &&
		(await redis.get('Xiuxian:PlayerRecord3'))
	);
}
