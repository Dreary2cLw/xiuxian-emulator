		...
		if (now_level_id >= 21 && now_level_id < 42) {
			...
			if (BossMax != null) {
				...
				if (msgg.find((item) => item == A_win)) {
					...
					e.reply(
						A_player.名号 + '击败了' + B_player.名号 + '，得到了' + BossMax.money + '灵石'
					);
				} else if (msgg.find((item) => item == B_win)) {
					//输了，被抢了灵石
					if (player.灵石 > BossMax.money) {
						await Add_灵石(usr_qq, -BossMax.money);
						e.reply(
							A_player.名号 + '打不过' + B_player.名号 + '，被抢走了' + BossMax.money + '灵石'
						);
					} else {
						await Add_灵石(usr_qq, -player.灵石 + 1);
						e.reply(
		...
		if (now_level_id < 21) {
			...
			if (BossMini != null) {
				...
				if (msgg.find((item) => item == A_win)) {
					...
					e.reply(
						A_player.名号 + '击败了' + B_player.名号 + '，得到了' + BossMini.money + '灵石'
					);
				} else if (msgg.find((item) => item == B_win)) {
					//输了，被抢了灵石
		...
		if (now_level_id < 999) {
			...
			if (sanbing != null) {
				...
				if (msgg.find((item) => item == A_win)) {
					...
					e.reply(
						A_player.名号 + '击败了' + B_player.名号 + '，得到了' + sanbing.money + '灵石'
					);
				} else if (msgg.find((item) => item == B_win)) {
					//输了，被抢了灵石
...
export async function BossCD(e) {
	...
	if (now_time < Bosstime + transferTimeout) {
		...
		e.reply(
			`每${transferTimeout / 1000 / 60}分钟讨伐一次。` + `cd: ${game_m}分${game_s}秒`
		);
		//存在CD。直接返回1
		return 1;
	}
