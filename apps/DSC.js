			...
			while (CurrentPlayerAttributes.当前血量 > 0 && bosszt.Health > 0) {
				...
				if (!(BattleFrame & 1)) {
					...
					if (
						...
						CurrentPlayerAttributes.学习的功法.indexOf('剑帝一剑') > -1 &&
						BattleFrame == 2
					) {
					...
					msg.push(
						`${CurrentPlayerAttributes.名号}损失血量${BOSS_To_Player_Damage}，${CurrentPlayerAttributes.名号}剩余血量${CurrentPlayerAttributes.当前血量}`
					);
				}
