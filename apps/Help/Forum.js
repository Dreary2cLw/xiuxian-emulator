...
async function Pu(e, x) {
	...
	if (time <= 0) {
		...
		let najieNumber = await exist_najie_thing(
			usr_qq,
			thingless.thing.name,
			thingless.thing.class,
			pj
		);
...
export class Forum extends plugin {
	...
	async off(e) {
		...
		await genxinliebiao(F);
		e.reply(
			'成功取消该委托，已归还委托金' + guihuan + '收取委托管理费用' + guihuan * 0.1
		);
	}
	...
	async Pushforum(e) {
		...
		// 价格阈值设定
		if (
			thing_value <= thing_exist.出售价 * 0.8 * thing_amount &&
			thing_exist.出售价 != 1
		) {
			e.reply('价格过低');
			return;
