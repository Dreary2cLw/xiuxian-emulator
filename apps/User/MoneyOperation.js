...
export class MoneyOperation extends plugin {
	constructor() {
		super({
			...
			rule: [
				...
				{
					reg:
						'^#发(装备|道具|丹药|功法|草药|材料|盒子|仙宠|口粮|项链|食材).*\\*-?[1-9]d*',
					fnc: 'wup',
				},
				{
					reg:
						'^#全体发(装备|道具|丹药|功法|草药|材料|盒子|仙宠|口粮|项链|食材).*\\*[1-9]d*',
					fnc: 'wup_all',
				},
				{
					reg: '^#扣除.*$',
	...
	async wup_all(e) {
		...
		e.reply(
			`发放成功,目前共有${File_length}个玩家,每人增加${amount}个${thing_name}`
		);
	}
		...
		if (msg.startsWith('灵石')) {
			...
			if (number >= amount) {
				...
				e.reply([
					segment.at(A_qq),
					segment.at(B_qq),
					`${B_player.名号} 获得了由 ${A_player.名号}赠送的[${thing_name}]×${amount}`,
				]);
			} else {
