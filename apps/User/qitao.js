	...
	async qt(e) {
		...
		if (player.灵石 < 100) {
			if (random4 > 0.5) {
				...
				await sleep(5000);
				e.reply(
					'钟大爷发现在旁吃瓜的你，对你说:‘你有带灵石吗’（至少带100灵石给钟大爷付账）'
				);
				return;
			} else {
				e.reply(
					'遇到了想喝牛奶团子的雷电影，发现你没有灵石买，人偶上号了，一刀劈了过来，幸好跑得快躲过了一劫' +
						'\n' +
						'（至少带100灵石给影买团子牛奶）'
				);
				return;
		...
		if (random > 0.01) {
			...
			if (player.灵石 > 50000) {
				e.reply(
					'遇到喝酒没钱付账的吟游诗人，被酒馆认出你两是朋友，被迫替他付清了酒钱50000灵石'
				);
			...
			if (player.灵石 > 20000) {
				e.reply(
					'不知道哪里飞出来的野狗到处咬人，把你创翻在地并咬伤了你，去医院花了10000灵石'
				);
			...
			if (player.灵石 > 1000) {
				e.reply(
					'一群飞剑党呼得一下飞过，把你碗里的一半灵石都给抢走了，剩下一半洒落一地,你扬天长啸为何命运如此不公！！然后闪着腰了，去医院治疗花光了所有灵石'
				);
				...
				if (random3 > 0.5) {
					e.reply(
						'突然有个叫虎哥的人和你说:“来小亮给他整个活！”，你不受控制得：草！走！忽略！，身上的灵石都被你后空翻甩飞了，正好一群广场舞大妈跳完广场舞路过，哄抢一通，身上一分都没有了'
					);
					await Add_灵石(usr_qq, -player.灵石);
					return;
				} else {
					e.reply(
						'你在乞讨的时候，看到有怪兽出现，此时有一位小哥说他是铠甲勇士，但是需要你给他100灵石作为能量，你把家底递给了他，谁知小哥大喊一声:铠甲勇士合体！就抱着怪兽跑了'
					);
					await Add_灵石(usr_qq, -player.灵石);
					return;
				}
